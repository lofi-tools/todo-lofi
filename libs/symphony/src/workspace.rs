...
impl WorkspaceManager {
    ...
    pub fn get_or_create_workspace(&self, identifier: &str) -> Result<PathBuf> {
        ...
        if !workspace_exists {
            fs::create_dir_all(&workspace_path).map_err(|e| WorkspaceError {
                source: Box::new(e),
            })?;
    ...
    pub fn remove_workspace(&self, identifier: &str) -> Result<()> {
        ...
        if workspace_path.is_dir() {
            ...
            fs::remove_dir_all(&workspace_path).map_err(|e| WorkspaceError {
                source: Box::new(e),
            })?;
        }
    ...
    pub fn validate_path(&self, path: &Path) -> Result<()> {
        let canonical_path = dunce::canonicalize(path).map_err(|e| WorkspaceError {
            source: Box::new(e),
        })?;
        let canonical_root = dunce::canonicalize(&self.root).map_err(|e| WorkspaceError {
            source: Box::new(e),
        })?;
