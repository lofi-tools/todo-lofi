...
pub struct Agent {
    ...
    /// Whether the runner's once-per-session F-08 "answered without tools"
...
impl AgentBuilder {
    ...
    /// Whether the runner may force a follow-up turn when the model answers
    /// without calling any tool (the F-08 nudge). Default `true`. Disable for
    /// model families where chat-style, tool-free answers are expected — a
    /// forced tool round after a complete answer is how "the agent won't
    /// stop" manifests on adaptive-thinking models.
    pub fn no_tool_nudge(mut self, enabled: bool) -> Self {
        self.no_tool_nudge = enabled;
        self
    }
...
mod tests {
    ...
    #[test]
    fn no_tool_nudge_defaults_on_and_can_be_disabled() {
        // Default keeps upstream behavior (nudge may fire).
        let agent = Agent::builder().provider(StubProvider).build().unwrap();
        assert!(agent.no_tool_nudge);
        // Chat-style families opt out so a complete answer isn't followed by
        // a forced tool-use round.
